#!/usr/bin/env python3
"""
Simplified left Joy-Con control for SO100/SO101 robot
Fixed action format conversion issues
Uses P control, left Joy-Con changes target joint angles
"""
...
class FixedAxesLeftJoyconRobotics(JoyconRobotics):
    def common_update(self):
        # 修改后的更新逻辑：左手柄摇杆只控制固定轴向
        speed_scale = 0.0008
        
        # 垂直摇杆：只控制X轴（前后）- 使用左手柄左摇杆垂直方向
        joycon_stick_v = self.joycon.get_stick_left_vertical()
        ...
        # 水平摇杆：只控制Y轴（左右）- 使用左手柄左摇杆水平方向
        joycon_stick_h = self.joycon.get_stick_left_horizontal()
        ...
        # Z轴只通过左手柄按钮控制
        # L按钮控制Z轴上升
        joycon_button_up = self.joycon.get_button_l()
        if joycon_button_up == 1:
            self.position[2] += speed_scale * self.dof_speed[2] * self.direction_reverse[2]
        
        # 左摇杆按钮控制Z轴下降
        joycon_button_down = self.joycon.get_button_l_stick()
        if joycon_button_down == 1:
            self.position[2] -= speed_scale * self.dof_speed[2] * self.direction_reverse[2]

        # 其他按钮控制（左手柄映射）
        # 上方向键控制向前
        joycon_button_xup = self.joycon.get_button_up()
        # 下方向键控制向后
        joycon_button_xback = self.joycon.get_button_down()
        ...
        # Capture按钮重置位置（左手柄）
        joycon_button_home = self.joycon.get_button_capture()
        ...
        for event_type, status in self.button.events():
            ...
            if event_type == 'minus' and status == 1:
                ...
                self.reset_joycon()
            # 左扳机键控制夹爪
            elif event_type == 'zl' and not self.change_down_to_gripper:
                self.gripper_toggle_button = status
            # 左摇杆按钮作为备选夹爪控制
            elif event_type == 'stick_l_btn' and self.change_down_to_gripper:
                self.gripper_toggle_button = status
            else: 
                self.reset_button = 0
...
def p_control_loop(
    robot, keyboard, target_positions, start_positions, current_x, current_y, joyconrobotics_left, kp=0.5, control_freq=50
):
    """
    P control loop
    ...
    Args:
        ...
        joyconrobotics_left: left joycon robotics instance
        kp: proportional gain
        control_freq: control frequency (Hz)
    """
    control_period = 1.0 / control_freq
    ...
    while True:
        try:
            ...
            # 获取左手柄控制信息
            pose, gripper, control_button = joyconrobotics_left.get_control()
...
def main():
    """Main function"""
    print("LeRobot Simplified Left Joy-Con Control Example (P Control)")
    print("=" * 50)
    ...
    try:
        ...
        keyboard.connect()
        # 使用修改后的左手柄控制类
        joyconrobotics_left = FixedAxesLeftJoyconRobotics(
            "left",
            dof_speed=[2, 2, 2, 1, 1, 1]
        )

        print("左手柄固定轴向控制测试:")
        print("左摇杆垂直方向: 只控制X轴（前后）")  
        print("左摇杆水平方向: 只控制Y轴（左右）")
        print("L按钮: Z轴上升")
        print("左摇杆按钮: Z轴下降")
